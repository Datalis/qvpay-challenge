...
try:
    ...
    grouped_data.columns = [
        'Moneda', 'Fecha',
        'Oferta total de compra', 'Precio medio de compra del USDT', 'Varianza del precio de compra',
        'Oferta total de venta', 'Precio medio de venta del USDT', 'Varianza del precio de venta'
    ]
...
# Layout de la app
app.layout = html.Div([
    html.H1("Transaction Analysis Dashboard", style={'text-align': 'center', 'font-family': 'Calibri'}),
    html.Div([
...
def update_dashboard(selected_coin, selected_date, selected_type):
    ...
    fig1 = px.scatter(
        ...
        x='amount',
        y='price',
        title="Ofertas según tipo en el día",
        color='type',
        #labels={'amount':'Monto de la oferta','price':'Precio del USDT'}
    )

    # Datos de la tabla
    filtered_table_data = grouped_data[grouped_data['Fecha'] == selected_date].to_dict('records')
    logger.info(f"Filtered data size: {len(filtered_data)}, Table data size: {len(filtered_table_data)}")
    return fig1, filtered_table_data
